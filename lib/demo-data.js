...
const paginationTestProducts = process.env.SHOW_PAGINATION_TEST_DATA === "true"
  ? Array.from({ length: 12 }, (_, index) => {
      const number = String(index + 1).padStart(2, "0");
      const name = `Pagination Test Remote ${number}`;

      return {
        id: `pagination-test-${number}`,
        name,
        slug: `pagination-test-remote-${number}`,
        modelNumber: `TEST-${number}`,
        shortDescription: "Local pagination test product. Safe to remove after verification.",
        description: "This product exists only to verify catalogue pagination in the local preview.",
        status: "PUBLISHED",
        featured: false,
        sortOrder: 100 + index,
        categoryId: "demo-remote",
        category: demoCategories[0],
        images: [{
          id: `pagination-test-image-${number}`,
          url: "/assets/67d6ba3c0e8ef810.jpg",
          alt: name,
          sortOrder: 0,
        }],
        specifications: [],
